...
var transport = ({provider, input, input:{params, state, session}, output}) => {
  const callbackUrl = new URL(provider.callback || '/', 'https://example.org/');
  const callback = callbackUrl.pathname;
  const callbackQueryParams = callbackUrl.searchParams.toString();
  return {
    location:
      (params.override !== 'callback' && !output.error)
        ...
        : (!provider.transport || provider.transport === 'querystring')
          ? `${callback}?${qs.stringify(output)}${ callbackQueryParams ? '&' + callbackQueryParams : '' }`
